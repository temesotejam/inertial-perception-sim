...
def run_simulation(scenario='combined',duration=10.,seed=42,camera_enabled=True,range_enabled=True,imu_hz=200,camera_hz=15,range_hz=15):
    ...
    last_cam=last_range=last_cam_gt=last_range_gt=None
    prev_cam=None;prev_cam_reference=None;last_visual=None
    prev_range=None;prev_range_reference=None;last_range_frontend=None
    for k in range(int(round(duration*imu_hz))+1):
        t=k*dt;gt=truth_at(t,scenario);est.propagate(imu.sample(gt));event={"kind":"imu","residual_deg":0.,"correction_deg":0.};camera_sampled=range_sampled=False
        ...
        if range_enabled and t+1e-9>=next_range:
            ...
            if rr is not None and prev_range_reference is not None:
                est.update_range_relative(rr["rotation"],prev_range_reference,rr["pairs"],rr["range_rms_m"],rr["translation_m"]);event=est.last_event.copy();event["range_prev_points"]=rr["prev_points"];event["range_current_points"]=rr["current_points"];last_range_frontend=event.copy()
            prev_range=last_range;range_sampled=True;next_range+=1/range_hz
        # Each relative frontend references the fully fused state at its sample time.
        ...
        if k%max(1,int(imu_hz/20))==0:
            ...
            if last_range is not None:
                ...
                # World points are exported for visualization/evaluation only and are never consumed by the estimator.

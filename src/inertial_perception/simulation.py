...
def run_simulation(scenario='combined',duration=10.,seed=42,camera_enabled=True,range_enabled=True,imu_hz=200,camera_hz=15,range_hz=15,estimator_kind='ins_eskf'):
 ...
 for k in range(int(round(duration*imu_hz))+1):
  ...
  if camera_enabled and t+1e-9>=next_cam:
   ...
   if vis is not None and prev_cam_reference is not None:
    ...
    guarded_parallax=bool(vis.get('parallax_detected',False))
 ...
 return {'meta':{'imu_hz':imu_hz,'camera_hz':camera_hz,'range_hz':range_hz,'estimator_kind':estimator_kind,'state':'p,v,q,b_g,b_a' if hasattr(est,'position') else 'q,b_g','camera':{'width':cam.width,'height':cam.height,'fov_deg':cam.fov_deg,'render_width':cam.render_width,'render_height':cam.render_height,'feature_detector':'Harris','tracker':'dense Harris + global patch assignment + inertial-prior gating','constraint':'relative attitude; yaw-only whenever parallax is detected + cloned-pose bias shadow','clone_bias_mode':'shadow'},'range':{'rows':rngs.rows,'cols':rngs.cols,'tilt_down_deg':rngs.tilt_down_deg,'tracker':'local-plane temporal normal + ICP health check','constraint':'relative_tilt + normal_translation','translation_observability':'local_plane_normal_only','absolute_floor_prior':False},'scene':SCENE},'metrics':metrics,'records':records}

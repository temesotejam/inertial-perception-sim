...
def run_simulation(scenario='combined',duration=10.,seed=42,camera_enabled=True,range_enabled=True,imu_hz=200,camera_hz=15,range_hz=15,estimator_kind='ins_eskf'):
 ...
 for k in range(int(round(duration*imu_hz))+1):
  ...
  if range_enabled and t+1e-9>=next_range:
   ...
   if rr is not None and prev_range_reference is not None:
    est.update_range_relative(rr['previous_normal'],rr['current_normal'],prev_range_reference,rr['pairs'],rr['range_rms_m'],rr['translation_m'],float(np.degrees(rr['rotation'].magnitude())));event=est.last_event.copy();event['range_prev_points']=rr['prev_points'];event['range_current_points']=rr['current_points'];event['range_observable']=rr['observable'];last_range_frontend=event.copy()
  ...
  if hasattr(est,'position'):
   rec.update({'true_position':gt.position.tolist(),'est_position':s.position.tolist(),'position_error_m':float(np.linalg.norm(s.position-gt.position)),'true_velocity':gt.velocity.tolist(),'est_velocity':s.velocity.tolist(),'velocity_error_mps':float(np.linalg.norm(s.velocity-gt.velocity)),'accel_bias':s.accel_bias.tolist()})
 ...
 if hasattr(est,'position'):
  pe=np.array([r['position_error_m'] for r in records]);ve=np.array([r['velocity_error_mps'] for r in records]);metrics.update({'position_rmse_m':float(np.sqrt(np.mean(pe**2))),'velocity_rmse_mps':float(np.sqrt(np.mean(ve**2))),'final_position_error_m':float(pe[-1]),'final_velocity_error_mps':float(ve[-1]),'final_accel_bias':est.accel_bias.tolist()})
 return {'meta':{'imu_hz':imu_hz,'camera_hz':camera_hz,'range_hz':range_hz,'estimator_kind':estimator_kind,'state':'p,v,q,b_g,b_a' if hasattr(est,'position') else 'q,b_g','camera':{'width':cam.width,'height':cam.height,'fov_deg':cam.fov_deg,'render_width':cam.render_width,'render_height':cam.render_height,'feature_detector':'Harris','tracker':'2D patch + rotation RANSAC','constraint':'relative_rotation_from_image_tracks'},'range':{'rows':rngs.rows,'cols':rngs.cols,'tilt_down_deg':rngs.tilt_down_deg,'tracker':'local-plane temporal normal + ICP health check','constraint':'relative_tilt_only_preserve_yaw','absolute_floor_prior':False},'scene':SCENE},'metrics':metrics,'records':records}

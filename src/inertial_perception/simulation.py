...
def run_simulation(scenario='combined',duration=10.,seed=42,camera_enabled=True,range_enabled=True,imu_hz=200,camera_hz=15,range_hz=15,estimator_kind='ins_eskf'):
 ...
 last_cam=last_range=last_cam_gt=last_range_gt=None;prev_cam=None;prev_cam_reference=None;prev_cam_position_reference=None;last_visual=None;prev_range=None;prev_range_reference=None;prev_range_position=None;last_range_frontend=None
 for k in range(int(round(duration*imu_hz))+1):
  ...
  if camera_enabled and t+1e-9>=next_cam:
   last_cam_gt=gt;last_cam=cam.sample(gt);visual_prior=prev_cam_reference.inv()*est.orientation if prev_cam_reference is not None else None
   translation_prior=None
   if prev_cam_reference is not None and prev_cam_position_reference is not None and hasattr(est,'position'):
    d_world=est.position-prev_cam_position_reference;translation_prior=prev_cam_reference.inv().apply(d_world)
   vis=visual_relative_rotation(prev_cam,last_cam,cam,rotation_prior=visual_prior,translation_prior=translation_prior)
  ...
  if camera_sampled:
   ...
   if hasattr(est,'position'):prev_cam_position_reference=est.position.copy()

...
def _range_to_camera_overlay(frame,cam,range_gt,camera_gt):
    """Register range hits at their measurement time into a specific camera frame."""
    out=[]
...
def run_simulation(scenario='combined',duration=10.,seed=42,camera_enabled=True,range_enabled=True,imu_hz=200,camera_hz=30,range_hz=15):
    ...
    for k in range(int(round(duration*imu_hz))+1):
        ...
        if camera_enabled and t+1e-9>=next_cam:
            last_cam_gt=gt; last_cam=cam.sample(gt); obs=visual_orientation(last_cam,cam,gt.position)
        ...
        if k%max(1,int(imu_hz/20))==0:
            if last_cam is not None:
                rec["camera_timestamp"]=float(last_cam.timestamp)
                rec["camera_pose_euler"]=_euler_deg(last_cam_gt.orientation)
                rec["camera_render_size"]=[last_cam.render_width,last_cam.render_height]
                rec["camera_rgb_b64"]=base64.b64encode(last_cam.image_rgb.tobytes()).decode('ascii')
                rec["camera_features"]=[[f.u,f.v,f.feature_id,f.confidence] for f in last_cam.features]
            if last_range is not None:
                ...
                for r in last_range.rays:
                    if np.isfinite(r.distance):pts.append((last_range_gt.position+last_range_gt.orientation.apply(r.direction*r.distance)).tolist())
                rec["range_world_points"]=pts
                if last_cam is not None:
                    rec["range_camera_overlay"]=_range_to_camera_overlay(last_range,cam,last_range_gt,last_cam_gt)
                    rec["range_camera_dt_ms"]=float((last_range.timestamp-last_cam.timestamp)*1000.)
        records.append(rec)
    ...
    return {"meta":{"imu_hz":imu_hz,"camera_hz":camera_hz,"range_hz":range_hz,"camera":{"width":cam.width,"height":cam.height,"fov_deg":cam.fov_deg,"render_width":cam.render_width,"render_height":cam.render_height,"feature_detector":"Harris"},"range":{"rows":rngs.rows,"cols":rngs.cols,"tilt_down_deg":rngs.tilt_down_deg},"scene":SCENE},"metrics":metrics,"records":records}

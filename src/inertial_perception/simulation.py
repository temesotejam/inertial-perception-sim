 ...
 for k in range(int(round(duration*imu_hz))+1):
  ...
  if camera_enabled and t+1e-9>=next_cam:
   ...
   if vis is not None and prev_cam_reference is not None:
    ...
    if hasattr(est,'position'):est.update_camera_relative(vis['rotation'],prev_cam_reference,vis['tracks'],vis['track_rms_deg'],parallax_detected=vis.get('parallax_detected',False),range_tilt_supported=range_tilt_support)

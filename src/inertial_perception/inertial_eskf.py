...
class InertialESKF:
    ...
    def update_camera_relative(self,relative_rotation,reference_orientation,tracks=0,track_rms_deg=float('nan'),parallax_detected=False):
        ...
        if parallax_detected:
            # Our educational frontend does not yet solve a full, robust visual
            # relative pose at low resolution. Under detected parallax, the
            # direct rotation fit's tilt components are unreliable. Preserve the
            # gravity-orthogonal tilt from IMU/Range and use Camera only for the
            # world-gravity-axis component (heading/yaw information).

...
class InertialESKF:
    """15-state INS ESKF with optional Camera-attitude and Range-position clones.

    Core error state: [dp,dv,dtheta,dbg,dba]. Camera adds a 3-state historical
    attitude clone (used as bias shadow by default). Range adds a 3-state
    historical position clone so a metric normal displacement can be treated as
    a genuine relative measurement instead of an absolute-position pseudo update.
    """
    CORE_N=15
    def __init__(self,initial_position=None,initial_velocity=None,initial_orientation=None,
        ...
        self.camera_clone_orientation=None;self.camera_clone_slice=None;self.range_clone_position=None;self.range_clone_slice=None
        self.last_event={'kind':'init','residual_deg':0.,'correction_deg':0.,'filter':'ins_eskf'}

    @property
    def has_camera_clone(self):return self.camera_clone_orientation is not None and self.camera_clone_slice is not None
    @property
    def has_range_clone(self):return self.range_clone_position is not None and self.range_clone_slice is not None
    ...
    def _remove_camera_clone(self):
        if self.camera_clone_slice is None:return
        s=self.camera_clone_slice;idx=np.arange(s.start,s.stop);self.P=np.delete(np.delete(self.P,idx,axis=0),idx,axis=1)
        if self.range_clone_slice is not None and self.range_clone_slice.start>s.start:
            rs=self.range_clone_slice;self.range_clone_slice=slice(rs.start-3,rs.stop-3)
        self.camera_clone_slice=None;self.camera_clone_orientation=None

    def _remove_range_clone(self):
        if self.range_clone_slice is None:return
        s=self.range_clone_slice;idx=np.arange(s.start,s.stop);self.P=np.delete(np.delete(self.P,idx,axis=0),idx,axis=1)
        if self.camera_clone_slice is not None and self.camera_clone_slice.start>s.start:
            cs=self.camera_clone_slice;self.camera_clone_slice=slice(cs.start-3,cs.stop-3)
        self.range_clone_slice=None;self.range_clone_position=None

    def _augment_clone(self,J):
        J=np.asarray(J,float);cross=self.P@J.T;cov=J@self.P@J.T
        self.P=np.block([[self.P,cross],[cross.T,cov]]);self.P=(self.P+self.P.T)*.5
        return slice(self.P.shape[0]-J.shape[0],self.P.shape[0])
    ...
    def _inject(self,dx):
        ...
        if self.has_camera_clone:self.camera_clone_orientation=Rotation.from_rotvec(dx[self.camera_clone_slice])*self.camera_clone_orientation
        if self.has_range_clone:self.range_clone_position+=dx[self.range_clone_slice]
    ...
    def _normal_position_kalman(self,residual,n_world,sigma):
        ...
        var_n=float(n@self.P[:3,:3]@n);S=max(var_n+float(sigma)**2,1e-12);K=np.zeros((N,1));K[:3,0]=n*(var_n/S)
        dx=K[:,0]*float(residual);I=np.eye(N);A=I-K@H;R=np.array([[float(sigma)**2]]);self.P=A@self.P@A.T+K@R@K.T;self.P=(self.P+self.P.T)*.5;return dx
    ...
    def propagate(self,imu):
        if self.last_t is not None:
            ...
            N=self.P.shape[0];F=np.eye(N);F[:self.CORE_N,:self.CORE_N]=Fcore;Q=np.zeros((N,N));Q[:self.CORE_N,:self.CORE_N]=Qcore;self.P=F@self.P@F.T+Q;self.P=(self.P+self.P.T)*.5
    ...
    def update_camera_relative(self,relative_rotation,reference_orientation,tracks=0,track_rms_deg=float('nan'),parallax_detected=False,range_tilt_supported=False,enable_clone_bias=False):
        ...
        if self.has_camera_clone:
            Hclone=np.zeros((3,self.P.shape[0]));Hclone[:,6:9]=Proj;Hclone[:,self.camera_clone_slice]=-Proj;sdx=self._shadow_dx(residual,Hclone,Rcov);shadow_dbg=np.degrees(sdx[9:12]);shadow_att=np.degrees(sdx[6:9])
        if self.has_camera_clone and enable_clone_bias:H=Hclone;coupling='cloned_pose_relative_yaw' if use_yaw_only else 'cloned_pose_relative';active_rows=None
        else:
            H=np.zeros((3,self.P.shape[0]));H[:,6:9]=Proj;coupling='attitude_only_clone_shadow_yaw' if use_yaw_only and self.has_camera_clone else ('attitude_only_clone_shadow' if self.has_camera_clone else ('attitude_only_yaw_under_parallax' if use_yaw_only else 'attitude_only'));active_rows=np.arange(6,9)
        extra={'tracks':int(tracks),'track_rms_deg':float(track_rms_deg),'visual_rotation_deg':float(np.degrees(relative_rotation.magnitude())),'imu_rotation_deg':float(np.degrees(imu_delta.magnitude())),'visual_quality':float(quality),'measurement_sigma_deg':float(np.degrees(sigma)),'camera_state_coupling':coupling,'camera_clone_active':bool(self.has_camera_clone),'camera_clone_bias_enabled':bool(enable_clone_bias),'shadow_gyro_bias_delta_dps':shadow_dbg.tolist(),'shadow_attitude_delta_deg':shadow_att.tolist(),'parallax_detected':bool(parallax_detected),'range_tilt_supported':bool(range_tilt_supported),'raw_camera_residual_deg':float(np.degrees(np.linalg.norm(raw)))};self._update(residual,H,Rcov,'camera_relative',extra,active_rows=active_rows)
    ...
    def update_range_relative(self,previous_normal,current_normal,reference_orientation,pairs=0,range_rms_m=float('nan'),translation_m=float('nan'),range_rotation_deg=float('nan')):
        desired_world=reference_orientation.apply(np.asarray(previous_normal,float));current_world=self.orientation.apply(np.asarray(current_normal,float));corr=align_vector_correction(current_world,desired_world);residual=corr.as_rotvec();count_quality=max(.05,min(1.,(pairs-7)/16.));rms_quality=max(.05,min(1.,1.-range_rms_m/.12)) if np.isfinite(range_rms_m) else .05;translation_quality=max(.05,min(1.,1.-translation_m/.22)) if np.isfinite(translation_m) else .05;quality=count_quality*rms_quality*translation_quality;sigma=self.range_noise/max(np.sqrt(quality),.18);n=current_world/np.linalg.norm(current_world);H=np.zeros((3,self.P.shape[0]));H[:,6:9]=np.eye(3)-np.outer(n,n);imu_delta=reference_orientation.inv()*self.orientation;extra={'pairs':int(pairs),'range_rms_m':float(range_rms_m),'range_translation_m':float(translation_m),'range_rotation_deg':float(range_rotation_deg),'imu_rotation_deg':float(np.degrees(imu_delta.magnitude())),'range_quality':float(quality),'measurement_sigma_deg':float(np.degrees(sigma)),'range_state_coupling':'attitude_only'};self._update(residual,H,np.eye(3)*sigma**2,'range_relative',extra,active_rows=np.arange(6,9))

    def update_range_translation(self,normal_previous,displacement_m,reference_position,reference_orientation,quality=1.0,plane_rms_m=float('nan'),normal_angle_deg=float('nan'),enable_range_clone=True):
        n_prev=np.asarray(normal_previous,float);n_prev/=np.linalg.norm(n_prev);n_world=reference_orientation.apply(n_prev);n_world/=np.linalg.norm(n_world);q=float(np.clip(quality,.02,1.));sigma=self.range_translation_noise_m/max(np.sqrt(q),.18)
        before_p=self.position.copy();before_v=self.velocity.copy();before_q=self.orientation;before_bg=self.gyro_bias.copy();before_ba=self.accel_bias.copy()
        if enable_range_clone and self.has_range_clone:
            predicted=float(np.dot(n_world,self.position-self.range_clone_position));residual=float(displacement_m-predicted);H=np.zeros((1,self.P.shape[0]));H[0,:3]=n_world;H[0,self.range_clone_slice]=-n_world
            active=np.r_[np.arange(self.CORE_N),np.arange(self.range_clone_slice.start,self.range_clone_slice.stop)];dx=self._kalman(np.array([residual]),H,np.array([[sigma**2]],float),active_rows=active);coupling='cloned_relative_position'
        else:
            predicted=float(np.dot(n_world,self.position-np.asarray(reference_position,float)));residual=float(displacement_m-predicted);dx=self._normal_position_kalman(residual,n_world,sigma);coupling='normal_position_only'
        self._inject(dx);dp=self.position-before_p;normal_dp=float(np.dot(dp,n_world));tangent_dp=float(np.linalg.norm(dp-normal_dp*n_world));d=np.diag(self.P)[:15]
        self.last_event={'kind':'range_translation','residual_deg':0.,'correction_deg':float(np.degrees((self.orientation*before_q.inv()).magnitude())),'filter':'ins_eskf','range_translation_residual_m':residual,'range_translation_observed_m':float(displacement_m),'range_translation_predicted_m':predicted,'range_translation_sigma_m':float(sigma),'range_translation_quality':q,'range_translation_plane_rms_m':float(plane_rms_m),'range_translation_normal_angle_deg':float(normal_angle_deg),'range_translation_state_coupling':coupling,'range_position_clone_active':bool(self.has_range_clone),'position_correction_m':float(np.linalg.norm(dp)),'normal_position_correction_m':normal_dp,'tangential_position_correction_m':tangent_dp,'velocity_correction_mps':float(np.linalg.norm(self.velocity-before_v)),'gyro_bias_correction_dps':float(np.degrees(np.linalg.norm(self.gyro_bias-before_bg))),'accel_bias_correction':float(np.linalg.norm(self.accel_bias-before_ba)),'accel_bias_norm':float(np.linalg.norm(self.accel_bias)),'sigma_pos_m':float(np.sqrt(np.mean(d[:3]))),'sigma_vel_mps':float(np.sqrt(np.mean(d[3:6]))),'covariance_dim':int(self.P.shape[0])}

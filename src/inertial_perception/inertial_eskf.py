...
class InertialESKF:
    ...
    When a Camera frame is cloned, P is augmented to 18x18 with the historical
    attitude error. A Camera relative observation then constrains current
    attitude minus cloned attitude, preserving the correlation needed for a
    physically meaningful gyro-bias update.
    """
    CORE_N=15
    ...
    def _pad_H(self,H):
        ...
        if H.shape[1]==self.CORE_N and self.P.shape[0]>self.CORE_N:
            return np.hstack([H,np.zeros((H.shape[0],self.P.shape[0]-self.CORE_N))])
    ...
    def set_camera_clone(self):
        """Replace any old Camera clone with the current attitude and covariance."""
        ...
        self.P=np.block([[self.P,cross],[cross.T,clone_cov]])
        self.P=(self.P+self.P.T)*.5;self.camera_clone_orientation=self.orientation
    ...
    def update_camera_relative(self,relative_rotation,reference_orientation,tracks=0,track_rms_deg=float('nan'),parallax_detected=False,range_tilt_supported=False):
        ...
        if self.has_camera_clone:
            H=np.zeros((3,18));H[:,6:9]=Proj;H[:,15:18]=-Proj;coupling='cloned_pose_relative_yaw' if use_yaw_only else 'cloned_pose_relative';active_rows=None
        else:
            H=np.zeros((3,15));H[:,6:9]=Proj;coupling='attitude_only_yaw_under_parallax' if use_yaw_only else 'attitude_only';active_rows=np.arange(6,9)
        extra={"tracks":int(tracks),"track_rms_deg":float(track_rms_deg),"visual_rotation_deg":float(np.degrees(relative_rotation.magnitude())),"imu_rotation_deg":float(np.degrees(imu_delta.magnitude())),"visual_quality":float(quality),"measurement_sigma_deg":float(np.degrees(sigma)),"camera_state_coupling":coupling,"camera_clone_active":bool(self.has_camera_clone),"parallax_detected":bool(parallax_detected),"range_tilt_supported":bool(range_tilt_supported),"raw_camera_residual_deg":float(np.degrees(np.linalg.norm(raw)))}
        self._update(residual,H,np.eye(3)*sigma**2,"camera_relative",extra,active_rows=active_rows)
    ...
    def update_range_translation(self,normal_previous,displacement_m,reference_position,reference_orientation,quality=1.0,plane_rms_m=float('nan'),normal_angle_deg=float('nan')):
        ...
        # The Range normal-translation pseudo-measurement references a previous
        # Range state that is not part of this Camera clone. It may correct the
        # current 15-state INS core, but must not rewrite the historical Camera
        # attitude clone through cross-covariance.

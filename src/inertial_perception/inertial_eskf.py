...
class InertialESKF:
    """15-state INS ESKF with optional cloned-Camera bias shadow diagnostics."""
    CORE_N=15
    def __init__(self,initial_position=None,initial_velocity=None,initial_orientation=None,
        ...
        self.gyro_noise_std=float(gyro_noise_std);self.accel_noise_std=float(accel_noise_std);self.bias_rw_std=float(bias_rw_std);self.accel_bias_rw_std=float(accel_bias_rw_std);self.accel_gravity_noise=np.radians(accel_gravity_noise_deg);self.camera_noise=np.radians(camera_noise_deg);self.range_noise=np.radians(range_noise_deg);self.range_translation_noise_m=float(range_translation_noise_m);self.gravity_update=bool(gravity_update);self.camera_clone_orientation=None
    ...
    def set_camera_clone(self):
        ...
        J=np.zeros((3,self.CORE_N));J[:,6:9]=np.eye(3);cross=self.P@J.T;clone_cov=J@self.P@J.T;self.P=np.block([[self.P,cross],[cross.T,clone_cov]]);self.P=(self.P+self.P.T)*.5;self.camera_clone_orientation=self.orientation
    ...
    def propagate(self,imu):
        if self.last_t is not None:
            ...
            if self.has_camera_clone:F=np.block([[Fcore,np.zeros((15,3))],[np.zeros((3,15)),np.eye(3)]]);Q=np.zeros((18,18));Q[:15,:15]=Qcore
    ...
    def update_camera_relative(self,relative_rotation,reference_orientation,tracks=0,track_rms_deg=float('nan'),parallax_detected=False,range_tilt_supported=False,enable_clone_bias=False):
        ...
        else:H=np.zeros((3,15));H[:,6:9]=Proj;coupling='attitude_only_clone_shadow_yaw' if use_yaw_only and self.has_camera_clone else ('attitude_only_clone_shadow' if self.has_camera_clone else ('attitude_only_yaw_under_parallax' if use_yaw_only else 'attitude_only'));active_rows=np.arange(6,9)
